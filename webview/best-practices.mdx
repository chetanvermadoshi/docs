...
<body>
    <iframe src="https://embed.v2.doshi.app?token=YOUR_TOKEN" 
...
function DoshiWebView() {
  return (
    ...
    >
      <WebView
        source={{ uri: 'https://embed.v2.doshi.app?token=YOUR_TOKEN' }}
        style={{ flex: 1 }}
        scrollEnabled={false}
...
webView.webViewClient = object : WebViewClient() {
    override fun shouldOverrideUrlLoading(view: WebView?, request: WebResourceRequest?): Boolean {
        ...
        if (url.startsWith("http") && !url.contains("embed.v2.doshi.app")) {
...
const DevelopmentWebview = ({ userData }) => {
  const url = `https://embed.v2.doshi.app?token=${token}`;
...
export const doshiConfig = {
  ...
  production: {
    apiUrl: 'https://api.doshi.app',
    embedUrl: 'https://embed.v2.doshi.app',
